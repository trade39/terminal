...
SYMBOL_MAP = {  # yfinance/Polygon mappings
...
def fetch_av(symbol: str, api_key: str) -> Optional[pd.DataFrame]:
    try:
        ...
        df = pd.DataFrame(data).T.astype(float).rename(columns={'1. open': 'open', '2. high': 'high', '3. low': 'low',
...
def fetch_polygon(symbol: str, api_key: str) -> Optional[pd.DataFrame]:
    try:
        ...
        df = pd.DataFrame([{'timestamp': pd.Timestamp(a.timestamp), 'open': a.open, 'high': a.high,
